...
out, restore_var = mobilenet_v2_224(x_b, train_flag, statistic_builder, score_builder, gradient_builder, EMA=False)
...
with tf.Session() as sess:
    ...
    all_var.restore(sess, './checkpoints/prune_process_no_train.ckpt')

    show = sess.run(tf.get_collection('show'))

    def check_3(ll):
        sum = 0
        for l in ll:
            sum += np.sum(np.sign(np.sign(l) + 0.5) - 1)
        return sum
    ...
    if TRAIN:
        ...
        print('before prune 0 var: ', pre_num)
        sess.run(score_builder.update_masks_for_zero_var_op_list)

        cur_num = score_builder.remaining_activation(sess)
        print('now number: ', cur_num)
